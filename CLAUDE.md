...
**Sticky header backdrop**: on desktop `.site-header` is transparent so the brown bar and nav pill "float" over scrolling content; on mobile (≤640px) the margins are too tight for that — text visibly slid past the bar's edges — so the media query gives `.site-header` a solid `--cream` background there. Don't "clean up" that mobile background: it's load-bearing, not a leftover.
