...
**Mobile nav**: the hamburger menu is a pure-CSS checkbox hack (`#menu-toggle` input + its `<label>` in `_includes/header.html`), deliberately not JS-driven — navigation has to keep working even if scripts fail to load, unlike the two features above. `.site-nav` is toggled via `.menu-toggle-input:checked ~ .site-nav` in `style.scss`, which requires the input and the nav to be *siblings* (both direct children of `<header>`) for the `~` combinator to reach across `.header-bar` in between. The "Inloggen" button is duplicated in the markup as `.nav-login--bar` (shown in the header bar itself on desktop, where there's room) and `.nav-login--menu` (shown at the bottom of the mobile dropdown instead) — CSS just shows/hides whichever fits, per breakpoint in the `@media (max-width: 640px)` block.

**Gotcha — `.site-nav a` vs `.nav-login`**: every generic `.site-nav a { ... }` rule (base styles, hover, the mobile dropdown's own overrides) has higher CSS specificity (class+element) than the bare `.nav-login` class, because `.nav-login--menu` lives *inside* `.site-nav`. Concretely this once made the mobile "Inloggen" button's text and icon invisible — `color` silently lost to `.site-nav a`'s brown, rendering brown-on-brown. Every `.site-nav a...` selector must stay written as `.site-nav a:not(.nav-login)` (already done everywhere as of this writing) — if a future edit adds a new bare `.site-nav a` rule without that exclusion, re-check that the login button still renders visibly in the mobile dropdown.
